<template>
  <v-container>
    ...
    <v-row v-if="firstManifests.length != 0 && secondManifests.length != 0">
      <diff-viewer
        :firstTemplates="firstManifests"
        :secondTemplates="secondManifests"
      >
      </diff-viewer>
    </v-row>
  </v-container>
...
export default {
  ...
  methods: {
    async renderBothManifest() {
      ...
      const firstResponse = await this.renderManifests(
        this.firstValues,
        this.firstVersion
      );
      ...
      const secondResponse = await this.renderManifests(
        this.secondValues,
        this.secondVersion
      );
    ...
    async renderManifests(values, version) {
      ...
      return await api.renderManifest(
        this.repo,
        this.chart,
        version,
        escaptedValues
