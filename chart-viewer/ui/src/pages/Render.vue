<template>
  <v-container>
    ...
    <v-row v-if="values != '' && manifests.length == 0">
      ...
      <v-col cols="12">
        <p>Customize the <code>values.yaml</code> below</p>
        <prism-editor
          ...
          :readonly="false"
        />
        <v-btn block color="primary" dark @click="getManifest()" class="mt-2"
          >Render Manifest</v-btn
        >
      </v-col>
    </v-row>
...
export default {
  ...
  methods: {
    ...
    async getManifest() {
      ...
      const response = await api.renderManifest(
        this.repo,
        this.chart,
        this.version,
        values
      ...
      if (response.status == 500) {
        ...
        this.generatedCommand =
          "kubectl apply -f " +
          process.env.VUE_APP_API_SERVER_HOST +
          response.data.url;
      }
    },
